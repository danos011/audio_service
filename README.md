...
## Установка зависимостей (dev среда)
...
- Для поднятия docker-compose стека с несколькими клиентами используй скрипт `entrypoint_compose.sh`.
...
- Поднять сервер + 1 клиент (по умолчанию):

```
./entrypoint_compose.sh --build --up
```

- Поднять сервер + N клиентов:
...
- Изменить количество клиентов на лету:
